...
class SpaceStationGenerator:
    ...
    STATION_PREFIXES = ["ISS", "LGW", "MOP",
                        "ERS", "TMO", "ABR", "DSO", "SWM", "QCH"]
...
class CrewMissionGenerator:
    ...
    def generate_crew_member(self, member_id: str) -> Dict[str, Any]:
        ...
        rank_bonus = {"cadet": 0, "officer": 2,
                      "lieutenant": 5, "captain": 8, "commander": 12}
        years_experience = min(
            base_experience + rank_bonus[rank] + random.randint(-2, 3), 30)
        ...
        for i in range(count):
            ...
            if duration > 365:
                ...
                experienced_count = sum(
                    1 for member in crew if member["years_experience"] >= 5)
...
def main():
    ...
    for contact in contacts:
        ...
        print(
            f"  {contact['contact_id']}: {contact['contact_type']} at {contact['location']} - {verified}")
    ...
    for mission in missions:
        ...
        print(
            f"    Crew: {len(mission['crew'])} members, Duration: {mission['duration_days']} days")
